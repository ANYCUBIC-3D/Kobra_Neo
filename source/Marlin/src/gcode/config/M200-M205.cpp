  ...
  LOOP_XYZE(i) {
    if (parser.seen(axis_codes[i])) {
      ...
      planner.set_max_acceleration(a, parser.value_axis_units((AxisEnum)a));
	  	...
	  	if (!parser.seen("XYZE")) {
    SERIAL_ECHOPAIR("Maximum Acceleration: X", planner.settings.max_acceleration_mm_per_s2[X_AXIS]);
    SERIAL_ECHOPAIR(" Y", planner.settings.max_acceleration_mm_per_s2[Y_AXIS]);	
  	...
  	SERIAL_ECHOLNPAIR_P(" E", planner.settings.max_acceleration_mm_per_s2[E_AXIS]);
...
void GcodeSuite::M203() {
  ...
  LOOP_XYZE(i)
	...
	if (!parser.seen("XYZE")) {
    SERIAL_ECHOPAIR("Maximum feedrates: X", planner.settings.max_feedrate_mm_s[X_AXIS]);
    SERIAL_ECHOPAIR(" Y", planner.settings.max_feedrate_mm_s[Y_AXIS]);	
		SERIAL_ECHOPAIR(" Z", planner.settings.max_feedrate_mm_s[Z_AXIS]);	 
  	SERIAL_ECHOLNPAIR_P(" E", planner.settings.max_feedrate_mm_s[E_AXIS]);
  ...
  if (parser.seen('T')) planner.settings.min_travel_feedrate_mm_s = parser.value_linear_units();
    ...
    SERIAL_ECHOPAIR(" S", planner.settings.min_feedrate_mm_s);	
  	...
  	SERIAL_ECHOPAIR(" X", planner.max_jerk.x);
		SERIAL_ECHOPAIR(" Y", planner.max_jerk.y);
		SERIAL_ECHOPAIR(" Z", planner.max_jerk.z);
		SERIAL_ECHOLNPAIR_P(" E", planner.max_jerk.e);
	}
}
