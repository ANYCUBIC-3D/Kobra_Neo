...
void GcodeSuite::M125() {
 ...
 if(parser.seen('R')) is_runout = true;
 ...
 //runout
  if(is_runout) {
  	park_point.x = _MIN(5,current_position.x);
		TERN_(HAS_LCD_MENU, ui.pause_show_message(PAUSE_MESSAGE_CHANGING, PAUSE_MODE_PAUSE_PRINT));
  }

  else
  {
  	park_point.x = current_position.x;
  ...
  if (pause_print(retract, park_point, 0, false)) {
    ...
    TERN_(POWER_LOSS_RECOVERY, if (recovery.enabled) recovery.save(true,park_point.z));
    if (ENABLED(EXTENSIBLE_UI) || !sd_printing || show_lcd) {
      wait_for_confirmation(is_runout, 0);
