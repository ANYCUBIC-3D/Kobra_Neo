...
#define START_OF_UTF8_CHAR(C) (((C) & 0xC0u) != 0x80U)
  ...
  public:
    ...
    float axis_value(const AxisEnum axis) {
     float pos;
     pos=NATIVE_TO_LOGICAL(processing ? destination[axis] : SUM_TERN(IS_KINEMATIC, current_position[axis], offset), axis);
      return  pos;
    }
...
public:
  ...
  MarlinUI() {
    ...
    AC_bar_control = false;
    start_flag = 1;
  }
      static char  start_flag;
     static  bool AC_bar_control;
//     static void AC_bar_CT(bool data) { AC_bar_control = data; }
  ...
  static void clear_lcd();
	...
	static void probe_preheating_start() { preheating_start = true; preheating_stop =false;}
	static void probe_preheating_stop() { preheating_start = false; preheating_stop =true;}
...
private:
  ...
  #if HAS_WIRED_LCD
    static void draw_status_screen(char seclect);
    #if HAS_GRAPHICAL_TFT
