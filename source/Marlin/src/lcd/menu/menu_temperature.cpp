  ...
  void lcd_setTargetHotend() {
   ...
   static char fresh_flag;
    ...
    if (ui.should_draw()) {
        ...
        tft.add_bar(1, 1, ((SLIDER_LENGTH-2) * fresh_data) / thermalManager.hotend_max_target(0), 7, COLOR_GREEN);
...
void lcd_setTargetBED() 
{
   ...
   static char fresh_flag;
    ...
    if (ui.should_draw()) {
        ...
        tft.set_background(COLOR_PROGRESS_BG);
        tft.add_rectangle(0, 0, SLIDER_LENGTH, 9, COLOR_PROGRESS_FRAME);
        tft.add_bar(1, 1, ((SLIDER_LENGTH-2) * fresh_data) /BED_MAX_TARGET, 7, COLOR_GREEN);
