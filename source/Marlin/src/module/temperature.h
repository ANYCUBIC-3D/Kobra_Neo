...
class Temperature {
  ...
  public:
      ...
      static inline celsius_t hotend_max_target(const uint8_t e) { return heater_maxtemp[e] - (HOTEND_OVERSHOOT); }
    #endif
    #if HAS_HEATED_BED
  ...
  private:
...
#if WATCH_HOTENDS
      static hotend_watch_t watch_hotend[HOTENDS];   // If temperature controller is running
#endif
    ...
    #endif
#if HAS_HOTEND
          static temp_range_t temp_range[HOTENDS];   // If temperature controller is running
#endif
    ...
    FORCE_INLINE static int16_t degTargetHotend(const uint8_t E_NAME) {
      ...
      static void setTargetHotend(const int16_t celsius, const uint8_t E_NAME) {
        const uint8_t ee = HOTEND_INDEX;
          ...
          if (celsius) powerManager.power_on();
        #endif
      ...
      static inline celsius_t wholeDegBed()       { return (int16_t)(degBed() + 0.5f); }
      FORCE_INLINE static int16_t degTargetBed()  { return temp_bed.target; }
      FORCE_INLINE static bool isHeatingBed()     { return temp_bed.target > temp_bed.celsius || (temp_bed.target >0 && temp_bed.celsius < temp_bed.target) ; }
