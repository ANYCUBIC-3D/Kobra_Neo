...
FORCE_INLINE void probe_specific_action(const bool deploy) {
  ...
  void Probe::preheat_for_probing(const uint16_t hotend_temp, const uint16_t bed_temp) {
    ...
    TERN_(WAIT_FOR_NOZZLE_HEAT, if (hotend_temp) thermalManager.setTargetHotend(hotend_temp, 0));
    TERN_(WAIT_FOR_BED_HEAT,    if (bed_temp)    thermalManager.setTargetBed(bed_temp));
