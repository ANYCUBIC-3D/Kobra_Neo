...
bool ACTouch::last_written_mode; // Initialized by settings.load, 0 = Open Drain; 1 = 5V Drain
...
bool ACTouch::command(const ACTCommand cmd, const millis_t &ms) {
//  if (DEBUGGING(LEVELING)) SERIAL_ECHOLNPGM("BLTouch Command :", cmd);
  MOVE_SERVO(Z_PROBE_SERVO_NR, cmd);
  safe_delay(_MAX(ms, (uint32_t)ACTOUCH_DELAY)); // BLTOUCH_DELAY is also the *minimum* delay
  return triggered();
}

// Init the class and device. Call from setup().
void ACTouch::init(const bool set_voltage/*=false*/) {
...
//    }
    ...
    const bool should_set = last_written_mode != ENABLED(ACTOUCH_SET_5V_MODE);

  #endif

  if (should_set && set_voltage)
    mode_conv_proc(ENABLED(ACTOUCH_SET_5V_MODE));
}
...
bool ACTouch::triggered() { return PROBE_TRIGGERED(); }

bool ACTouch::deploy_proc() {
  // Do a DEPLOY
...
//      if (DEBUGGING(LEVELING)) DEBUG_ECHOLNPGM("BLTouch Recovery Failed");
  ...
  TERN_(ACTOUCH_FORCE_SW_MODE, _set_SW_mode());
...
void ACTouch::mode_conv_proc(const bool M5V) {
  /**
